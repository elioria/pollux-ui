...
// <%= field.name %> - <%= field.dataType %>
  {
    ...
    size: <%= widthNum %>,
    meta: {
      displayName: '<%= field.grdLabel || field.name %> (<%= field.dataType === 'double' ? 'doublePrecision' : field.dataType %>)',
    },
    cell: ({ row }) => {
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                {value !== null && value !== undefined ? value.toFixed(<%= field.dataType === 'real' ? 2 : 4 %>) : ''}
              </div>
            </TooltipTrigger>
            <TooltipContent>

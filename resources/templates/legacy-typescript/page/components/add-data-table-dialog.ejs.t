---
to: src/app/(private)/generated/<%= h.changeCase.param(name) %>/components/add-<%= h.changeCase.param(name) %>-dialog.tsx
force: true
---
<%
...
import { PlusIcon, CloudUploadIcon, Undo2Icon, HelpCircleIcon, DatabaseIcon } from 'lucide-react';
...
import { add<%= h.changeCase.pascal(name) %> } from '../actions/entityActions';
...
<%- include(cwd + '/_templates/pollux/partials/add-dialog-form-schema-zod.ejs.t', { name: name, fields: fields }) -%>

export function Add<%= h.changeCase.pascal(name) %>Dialog() {
  const [open, setOpen] = useState(false);
  const router = useRouter();
  const { canCreate } = useCrudCapabilities();
  <%- include(cwd + '/_templates/pollux/partials/add-dialog-form-use-form.ejs.t', { name: name, fields: fields }) -%>

  <%- include(cwd + '/_templates/pollux/partials/add-dialog-form-on-submit.ejs.t', { name: name, fields: fields }) -%>

  // Capability-aware UI: the server enforces `create` regardless.
  if (!canCreate) return null;

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="default" className="h-9 rounded-lg px-3.5 shadow-sm">
      ...
      <DialogContent className="flex max-h-[min(90dvh,880px)] w-[min(96vw,960px)] max-w-none flex-col gap-0 overflow-hidden rounded-2xl border bg-card p-0 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <DialogTitle className="font-editorial text-2xl tracking-[-0.02em] sm:text-3xl">
                <%= data.formAddTitle || h.changeCase.title(name) %>
              </DialogTitle>
            </div>
          </div>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-1 flex-col overflow-hidden">
            <div className="scrollbar-thin scrollbar-track-transparent scrollbar-thumb-gray-400 grid min-h-0 flex-1 grid-cols-1 gap-5 overflow-y-auto px-5 py-6 sm:grid-cols-2 sm:px-7">
              <%- include(cwd + '/_templates/pollux/partials/add-dialog-form-fields.ejs.t', { name: name, fields: fields }) -%>
            </div>
                ...
                <div className="flex flex-row gap-2">
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p><%= data.formAddButtonSaveTooltipText || 'Salvar' %></p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p><%= data.formAddButtonBackTooltipText || 'Cancelar' %></p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                  ...
                  <Popover>
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p><%= data.formAddButtonHelpTooltipText || 'Ajuda' %></p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                    <PopoverContent
                      ...
                      <div className="space-y-4">
                        <div className="border-b pb-2">
                          <h4 className="text-lg font-semibold text-primary">
                            <%= data.formAddButtonHelpTooltipText || 'Ajuda' %> - <%= h.changeCase.title(name) %>
                          </h4>
                          <p className="text-sm text-muted-foreground">
                            Use este formulário para adicionar novos <%= h.inflection.pluralize(h.changeCase.camel(h.customPluralize(name))) %> ao sistema.
                          </p>
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <%- include(cwd + '/_templates/pollux/partials/add-edit-dialog-form-help-txts.ejs.t', { name: name, data: data, fields: fields }) -%>

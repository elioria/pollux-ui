// SPEC-006 same-origin API proxy (SPEC-003 subset) — Astro server endpoint.
//
// Browser code calls `/api/pollux/api/generated/v2/<entity>...`; this endpoint
// validates the request against the GENERATED registry fragments (strict
// entity/method/query allowlist), rebuilds the upstream path from normalized
// segments (never by concatenating raw user input), strips cookies and client
// authorization, enforces byte limits + timeout + redirect rejection, and
// forwards a server-only bearer credential. The upstream URL and credential
// never reach client output.
//
// Authentication (SPEC-003 BFF): the upstream bearer comes from the HttpOnly
// `pollux_access` cookie set by /api/pollux/auth (./auth/[action].ts). On a
...
const resolveOperation = (
  ...
  if (method === 'GET' && !hasTail) return 'list';
  if (method === 'GET' && tail === 'capabilities') return 'capabilities';
  if (method === 'GET') return 'read';
  if (method === 'POST' && !hasTail) return 'create';
  if (method === 'PATCH' && hasTail && tail !== 'capabilities') return 'update';
  if (method === 'DELETE' && hasTail && tail !== 'capabilities') {
    return 'delete';
  }
  return null;
};
...
export const ALL: APIRoute = async ({ params, request, locals }) => {
  ...
  const segments = (params.path ?? '').split('/').filter((s) => s.length > 0);
  ...
  const apiUrl = envValue(locals, 'POLLUX_API_URL');
  if (!apiUrl) {
    return failure(
      'SERVICE_UNAVAILABLE',
  ...
  const timeoutRaw = Number(envValue(locals, 'POLLUX_API_TIMEOUT_MS'));
  const timeoutMs =
  ...
  const auth = resolveProxyAuth(
    ...
    envValue(locals, 'POLLUX_DEV_BEARER') ?? null
  );
  ...
  if (
    ...
    const defense = await checkMutationDefense(request, auth, {
      publicOrigin: envValue(locals, 'POLLUX_PUBLIC_ORIGIN') ?? null,
      sessionSecret: envValue(locals, 'POLLUX_SESSION_SECRET') ?? null,
    });
    if (!defense.ok) {
      return failure(
        'FORBIDDEN',

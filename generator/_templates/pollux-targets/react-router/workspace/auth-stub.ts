// SPEC-005 fixed BFF auth endpoints (resource route registered by the
// handwritten app/routes.ts at `api/pollux/auth/:action`). Thin wrapper:
// the whole SPEC-003 "Authentication topology" flow lives in the
// framework-neutral core (app/lib/pollux/bff-core.server.ts):
//
...
import { handleAuthAction, resolveBffEnv } from '../lib/pollux/bff-core.server';

const dispatch = (request: Request, action: string) =>
  handleAuthAction(
    action,
    request,
...
export function loader({ request, params }: LoaderFunctionArgs) {
  return dispatch(request, params.action ?? '');
}

// SPEC-005 same-origin API proxy (SPEC-003 subset) — React Router resource
// route, registered by the handwritten app/routes.ts at `api/pollux/*`.
//
...
// Server-only environment (react-router-serve / vite dev run on Node).
...
async function proxy(request: Request, splat: string): Promise<Response> {
  ...
  // endpoints live at ./api.pollux.auth.ts).
  if (
    segments.some((s) => {

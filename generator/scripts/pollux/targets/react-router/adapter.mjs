// SPEC-005 — React Router 7 target adapter ('react-router', EXPERIMENTAL).
//
// Lifecycle per scripts/pollux/targets/protocol.mjs:
//   inspect -> plan -> render -> format -> verify
//
// The workspace skeleton is `skeletons/remix` (React Router 7 framework
// mode; the skeleton keeps its historical 'remix' name as a
// backward-compatible alias — the adapter id is 'react-router'). Route
// registration is fs/glob driven and preserves handwritten files:
//
//   - app/routes.ts (handwritten) enumerates app/routes/pollux/<plural>/
//     directories via app/pollux-routes.mjs and spreads them after its own
//     routes;
//   - app/root.tsx (handwritten) collects app/generated/pollux/nav/*.ts
//     sidebar fragments via import.meta.glob;
//   - app/routes/api.pollux.$.ts (generated, shared) collects
//     app/generated/pollux/registry/*.ts allowlist fragments the same way.
//
...
// the static template files under _templates/pollux-targets/{react-router,
...
const TEMPLATE_DIR = path.join(
  REPO_ROOT,
  '_templates/pollux-targets/react-router/workspace'
);
...
// layout (app/lib/pollux/runtime); UI files move to app/components/pollux and
...
const SHARED_OUTPUTS = {
  'app/routes/api.pollux.$.ts': TPL.apiProxy,
  'app/routes/api.pollux.auth.ts': TPL.authStub,
  'app/lib/pollux/use-pollux-mutation.ts': TPL.mutationHook,
  // Framework-neutral BFF auth core; the `.server` suffix keeps it out of
  // client bundles by construction (React Router/Vite server-only modules).
  'app/lib/pollux/bff-core.server.ts': readTemplate(
...
const buildNavTs = (model) =>
  [
    `// Sidebar registry fragment — consumed by the handwritten app/root.tsx`,
    `// via import.meta.glob('./generated/pollux/nav/*.ts').`,
...
const buildRegistryTs = (model) => {
  ...
  return [
    ...
    `// app/routes/api.pollux.$.ts via import.meta.glob. The proxy only`,
...
const FORBIDDEN_IMPORTS = [
  // Cross-framework imports must never reach React Router output (SPEC-005).
  /from\s+['"]next(\/|['"])/,
  /from\s+['"]@tanstack\//,
  /from\s+['"]astro(\/|['"])/,
  /from\s+['"]@astrojs\//,
...
export const adapter = {
  ...
  plan(model, context) {
    ...
    const outputs = [
      {
        path: `app/routes/pollux/${plural}/index.tsx`,
        content: applyTokens(TPL.listPage, tokens),
      },
      {
        path: `app/routes/pollux/${plural}/new.tsx`,
        content: applyTokens(TPL.newPage, tokens),
      },
      {
        path: `app/routes/pollux/${plural}/edit.tsx`,
        content: applyTokens(TPL.editPage, tokens),
      },
      {
        path: `app/generated/pollux/${entity}/spec.ts`,
        content: buildSpecTs(model),
      },
      {
        path: `app/generated/pollux/nav/${entity}.ts`,
        content: buildNavTs(model),
      },
      {
        path: `app/generated/pollux/registry/${entity}.ts`,
  ...
  // (`react-router typegen && tsc --noEmit`) needs an installed workspace
  // (node_modules + generated route types) which the bare staging tree never
  // has — when a test stages into a full installed workspace copy the check
  // runs; otherwise the workspace-level `pnpm typecheck` covers it after
  // publication (the SPEC-005 canonical verification order).
  verify(staging) {
    const problems = [];
